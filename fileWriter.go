...
// 创建file writer
func NewFileWriter(fileName string) (fileWriter *FileWriter, err error) {
...
func (self *FileWriter) resetFile() (err error) {
...
func (self *FileWriter) write(level Level, format string) {
	self.lock.Lock()
	defer func() {
		self.lock.Unlock()
		// logrotate
...
// 边解析边输出
// 使用 % 作占位符
func (self *FileWriter) writef(level Level, format string, args ...interface{}) {
	self.lock.Lock()
	// 统计日志size
	var size int = 0
...
func (self *FileWriter) RotateSize() ByteSize {
...
func (self *FileWriter) RotateLine() int {
...
func (self *FileWriter) Colored() bool {
	return self.colored
}
...
func (self *FileWriter) SetHookLevel(level Level) {
